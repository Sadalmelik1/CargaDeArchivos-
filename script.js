...
function saveFile(fileName, content) {
  ...
  savedFiles.push({ name: fileName, content: content, id: Date.now() });
...
function displayFiles(files) {
  ...
  files.forEach((file) => {
    ...
    deleteButton.setAttribute("onclick", "mostrarFunc()")
...
function deleteFile(id) {
  ...
  savedFiles = savedFiles.filter((file) => file.id !== id);
  localStorage.setItem('files', JSON.stringify(savedFiles));
  displayFiles(savedFiles);
}
...
/*function mostrarFunc() {
  setTimeout(function () {
    var box = document.getElementById("MvBorder");
    var Yotesteo = document.getElementById("file-list");

    if (Yotesteo.innerHTML.trim() !== "") {
...
function inputFunction(){
  var inputArchivo = document.getElementById('file-input');
  var divOculto = document.getElementById('MvBorder');
  ...
  Factory.prototype = {
    ...
    addRibbon: function () {
      ...
      var ribbon = [],
        ...
        point3 = null,
        color = Math.round(random(168, 150, 168, 150)),
        delay = 0
